...
type ModuleFourENProps = {
  courseModule: CourseModule;
  isComplete: boolean;
  onComplete: () => void;
};

export function ModuleFourEN({ courseModule, isComplete, onComplete }: ModuleFourENProps) {
  return (
    <article className="space-y-8">
      <section className="rounded-3xl bg-harbor px-6 py-9 shadow-soft md:px-8 md:py-10">
        <p className="text-sm font-bold uppercase tracking-normal text-white">
          Part {courseModule.order} &middot; Completion
        </p>
        ...
        <p className="mt-5 max-w-3xl text-lg leading-8 text-white">
          You don't need to know everything. You need to know what to do when you don't know.
        </p>
      </section>
      ...
      <Card className="p-6 md:p-8">
        <h2 className="text-2xl font-extrabold text-ink md:text-3xl">
          You are ready to contribute safely
        </h2>
        <p className="mt-4 text-base leading-8 text-slate">
          You have completed the entire course. Congratulations!
        </p>
      ...
      {isComplete ? (
        <div className="sr-only" aria-live="polite">Part 4 is already completed.</div>
      ) : null}
    </article>
  );
}
