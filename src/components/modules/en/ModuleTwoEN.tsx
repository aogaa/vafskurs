...
type ModuleTwoENProps = {
  courseModule: CourseModule;
  isComplete: boolean;
  onComplete: () => void;
};

export function ModuleTwoEN({ courseModule, isComplete, onComplete }: ModuleTwoENProps) {
  return (
    <article className="space-y-8">
      ...
      {isComplete ? (
        <div className="sr-only" aria-live="polite">Part 2 is already completed.</div>
      ) : null}
    </article>
  );
}
