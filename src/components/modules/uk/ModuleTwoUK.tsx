...
type ModuleTwoUKProps = {
  courseModule: CourseModule;
  isComplete: boolean;
  onComplete: () => void;
};

export function ModuleTwoUK({ courseModule, isComplete, onComplete }: ModuleTwoUKProps) {
  return (
    <article className="space-y-8">
      ...
      {isComplete ? (
        <div className="sr-only" aria-live="polite">Розділ 2 вже завершено.</div>
      ) : null}
    </article>
  );
}
