...
type ModuleThreeUKProps = {
  courseModule: CourseModule;
  isComplete: boolean;
  onComplete: () => void;
};

export function ModuleThreeUK({ courseModule, isComplete, onComplete }: ModuleThreeUKProps) {
  return (
    <article className="space-y-8">
      ...
      {isComplete ? (
        <div className="sr-only" aria-live="polite">Розділ 3 вже завершено.</div>
      ) : null}
    </article>
  );
}
